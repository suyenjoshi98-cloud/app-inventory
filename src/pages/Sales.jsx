...
export default function Sales() {
  ...
  const exportToExcel = () => {
    const exportData = sales.flatMap((s) =>
      (s.items || []).map((item) => ({
  ...
  const handleProductSelect = (productName) => {
    ...
    if (product) {
      setItemRow({
        ...itemRow,
        product: productName,
        unitPrice: priceEntry?.sellingPrice || product.price || "",
      });
  ...
  const saveCustomer = (customerName, email, phone, address) => {
    ...
    if (index === -1) {
      ...
      if (email) existingCustomers[index].email = email;
      if (phone) existingCustomers[index].phone = phone;
      if (address) existingCustomers[index].address = address;
      if (existingCustomers[index].type === "First Time")
        existingCustomers[index].type = "Return";
    }
  ...
  const handleSubmit = () => {
    ...
    if (editId) {
      ...
      console.log("Customer email at submit:", customerEmail);
    ...
    saveCustomer(form.customer, customerEmail, customerPhone, customerAddress);
    setShowModal(false);
    setForm(emptyForm);
    setEditId(null);
    setStockAlert("");
  ...
  const openEdit = (s) => {
    ...
    const match = customers.find(
      (c) => c.name.toLowerCase() === s.customer.toLowerCase(),
    );
    if (match) {
      setCustomerInfo({
        type: match.type,
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  onBlur={() => {
                    ...
                    const match = customers.find(
                      (c) =>
                        c.name.toLowerCase() === form.customer.toLowerCase(),
                    );
                    if (match) {
                      setCustomerInfo({
                        type: match.type,
                ...
                {customerInfo && (
                  <div className="mt-1.5 text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2 flex gap-3">
                    <span>🏷️ {customerInfo.type}</span>
                    {customerInfo.email && <span>✉️ {customerInfo.email}</span>}
                    {customerInfo.phone && <span>📞 {customerInfo.phone}</span>}
                    {customerAddress.address && (
                      <span> 🏠︎ {customerAddress.address}</span>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="border border-gray-100 rounded-xl p-4 mb-4">
              ...
              {stockAlert && (
                <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 text-xs px-3 py-2 rounded-lg mb-3">
                  {stockAlert}
                </div>
              )}
              {stockError && (
